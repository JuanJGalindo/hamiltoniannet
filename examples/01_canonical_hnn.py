...
hnn_cfg = hnet.TrainConfig(epochs=EPOCHS, lr=1e-3, device=str(DEVICE), seed=SEED, log_every=1000)
...
print("-" * 42)
print(
    f"{'Baseline':<12} {baseline_metrics['rel_l2_q']:<14.4f} "
    f"{baseline_metrics['max_energy_error']:<14.4f}"
)
...
ax = axes[2]
ax.semilogy(
    baseline_trainer.history["loss"], "--", color="darkorange", label="Baseline MLP", alpha=0.8
)
